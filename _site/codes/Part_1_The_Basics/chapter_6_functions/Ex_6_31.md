...
<<div class="code-container">### Exercise 6.31: When is it valid to return a reference? A reference to const?-   return reference, when the parameter is a reference , usually function will be asked to return a position, so that it can be modified-   reference to const, when the parameter is a reference to const, usually to return something too big size</div></main><footer> 
    <nav class="footer">
        <ul>
